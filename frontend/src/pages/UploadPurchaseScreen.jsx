      ...
      >
        <div className="p-6 h-full overflow-y-auto">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-600 text-sm">Sell Item Dashboard</p>
          </div>
          ...
          <div className="mt-6 p-3 bg-white/60 backdrop-blur-sm border border-violet-200/40 rounded-lg text-center">
            ...
            <div className="text-xs font-semibold text-gray-700">Campus Marketplace</div>
            <div className="text-xs text-gray-600">Sell to fellow students</div>
          </div>
        </div>
      </div>
    </>
          ...
          <div className="mb-6 sm:mb-8 bg-white/90 backdrop-blur-lg border border-violet-200/50 p-4 sm:p-6 rounded-2xl shadow-xl">
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-xl sm:text-2xl font-bold text-gray-900">List Item for Sale</h1>
                  <p className="text-gray-600 text-sm">Help fellow students find great deals</p>
                </div>
              </div>
              <button
          ...
          <div className="max-w-2xl mx-auto">
            <div className="bg-white/90 backdrop-blur-lg border border-violet-200/50 rounded-2xl shadow-xl overflow-hidden">
              ...
              <div className="p-4 sm:p-6 max-h-[calc(100vh-280px)] sm:max-h-[calc(100vh-320px)] overflow-y-auto">
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={handleInputChange}
                          className="w-full pl-8 pr-3 sm:pr-4 py-3 sm:py-3.5 bg-white/80 border border-violet-200/60 rounded-lg sm:rounded-xl text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition-all duration-300 text-sm sm:text-base"
                          required
                        />
                      </div>
                    </div>
                  ...
                  {formData.oldPrice && formData.currentPrice && (
                    <div className="p-3 bg-green-50/80 backdrop-blur-sm border border-green-200/50 rounded-lg">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-gray-600">Savings for buyer:</span>
                        <span className="font-semibold text-green-600">
                          ₹{Math.max(0, parseFloat(formData.oldPrice) - parseFloat(formData.currentPrice))} 
                          ({Math.max(0, Math.round(((parseFloat(formData.oldPrice) - parseFloat(formData.currentPrice)) / parseFloat(formData.oldPrice)) * 100))}% off)
                        </span>
                      </div>
                    </div>
                  )}
                    ...
                    {images.length > 0 && (
                      <div className="mt-3 p-3 bg-violet-50/80 backdrop-blur-sm border border-violet-200/50 rounded-lg">
                        <p className="text-sm text-gray-600">
                          <span className="text-violet-600 font-semibold">{images.length}</span> image(s) selected
                        </p>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {Array.from(images).slice(0, 3).map((file, index) => (
                            <span key={index} className="inline-block px-2 py-1 bg-violet-100 text-violet-700 text-xs rounded-full">
            ...
            <div className="mt-6 p-4 bg-white/80 backdrop-blur-sm border border-violet-200/50 rounded-xl shadow-lg">
              <div className="text-center">
                <div className="text-2xl mb-2">💰</div>
                <h3 className="font-semibold text-gray-700 mb-1">Start Earning from Your Items</h3>
                <p className="text-sm text-gray-600">Turn your unused items into cash while helping fellow students find affordable deals!</p>
              </div>
            </div>
          </div>
        </div>
